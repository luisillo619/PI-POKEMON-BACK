...
const apiData = async () => {
  try {
    ...
    // const pokemonPromises = pokemonInfo.map(async (e) => {
    //   if (!namesSet.has(e.forms[0].name)) {
    //     namesSet.add(e.forms[0].name);
    //     const typesName = e.types.map((e) => e.type.name);
    //     const typesDb = await findOrCreateType(typesName);
